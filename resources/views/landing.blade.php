...
<head>
    ...
    <title>OrangeCrush Car Rental</title>
    <!-- Bootstrap (requested) -->
...
<body class="bg-white text-slate-900">
    <header class="relative">
        <img src="https://images.unsplash.com/photo-1503736334956-4c8f8e92946d?auto=format&fit=crop&w=2000&q=80" class="w-full h-[360px] object-cover" alt="Hero">
    ...
    <main class="relative z-10 max-w-6xl mx-auto px-4 -mt-6 md:-mt-10">
        ...
        <section id="browse" class="mb-14">
            <div class="flex items-center gap-3 mb-4">
                ...
                <span class="text-slate-500">{{ $cars->count() }} vehicles available</span>
            </div>
            ...
            <div class="grid md:grid-cols-3 gap-6">
                @foreach ($cars as $car)
                    ...
                    <article class="bg-white rounded-2xl shadow hover:shadow-lg transition overflow-hidden border border-slate-100">
                        <div class="relative">
                            <img src="{{ $img }}" alt="{{ $car->make }} {{ $car->model }}" class="w-full h-56 object-cover">
                            <span class="pill absolute top-4 right-4 bg-slate-900 text-white">{{ $type }}</span>
                        </div>
                        <div class="p-5 flex flex-col gap-3">
                            <div>
                                <h3 class="text-xl font-semibold">{{ $car->make }} {{ $car->model }}</h3>
                                <p class="text-slate-500">{{ $car->year }}</p>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                <span class="pill bg-slate-100 text-slate-800">Daily Rate: ${{ number_format($car->daily_rate, 0) }}</span>
                                <span class="pill bg-slate-100 text-slate-800">Status: {{ $car->status }}</span>
                            </div>
                            <div class="flex items-center justify-between pt-2">
                                <div>
                                    <div class="text-2xl font-bold">${{ number_format($car->daily_rate, 0) }}</div>
                                    <p class="text-sm text-slate-500">per day</p>
                                </div>
                                <a href="{{ route('booking.show', $car) }}" class="bg-slate-900 text-white px-4 py-2 rounded-full font-semibold hover:-translate-y-0.5 transition">Book Now</a>
                            </div>
                        </div>
                    </article>
                @endforeach
