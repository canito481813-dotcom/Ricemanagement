...
<head>
    ...
    <title>Book {{ $car->make }} {{ $car->model }}</title>
    <!-- Bootstrap (requested) -->
...
<body class="bg-white text-slate-900">
    <div class="max-w-6xl mx-auto px-4 py-8">
        <div class="mb-4">
            <a href="{{ url('/') }}" class="inline-flex items-center gap-2 text-slate-700 hover:underline">
        ...
        <div class="grid md:grid-cols-3 gap-6">
            ...
            <aside class="bg-white rounded-2xl shadow border border-slate-100 p-6">
                ...
                <img src="{{ $image }}" alt="{{ $car->make }} {{ $car->model }}" class="w-full h-40 object-cover rounded-lg mb-4">
                <div class="space-y-2">
                    <div>
                    ...
                    <div class="border-t border-slate-200 pt-3 mt-2 text-sm">
                        <div class="flex justify-between"><span>Rate per day</span><span>${{ number_format($car->daily_rate, 0) }}</span></div>
                    ...
                    <div class="flex justify-between items-center text-lg font-bold mt-1">
                        <span>Total</span><span>${{ number_format($car->daily_rate, 0) }}</span>
                    </div>
                    <div class="pt-3">
                        <p class="text-sm font-semibold mb-2">Features</p>
                        <div class="flex flex-wrap gap-2">
                            @foreach ($features as $feature)
                                <span class="px-3 py-1 bg-slate-100 rounded-full text-xs font-semibold text-slate-800">{{ $feature }}</span>
                            @endforeach
                        </div>
                    </div>
                </div>
